/* Software AES-128 implementation for non-x86 platforms */
#include "aes.h"

#if !AES_HW_AVAILABLE

static const uint8_t sbox[256] = {
 
};

static const uint8_t rsbox[256] = {
 
};

static const uint8_t rcon[11] = {0x8d,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};

static uint8_t xtime(uint8_t x) { return (x<<1) ^ (((x>>7)&1)*0x1b); }

static void sub_bytes(uint8_t *s) { for(int i=0;i<16;i++) s[i]=sbox[s[i]]; }
static void inv_sub_bytes(uint8_t *s) { for(int i=0;i<16;i++) s[i]=rsbox[s[i]]; }

static void shift_rows(uint8_t *s) {
    uint8_t t;
    t=s[1]; s[1]=s[5]; s[5]=s[9]; s[9]=s[13]; s[13]=t;
    t=s[2]; s[2]=s[10]; s[10]=t; t=s[6]; s[6]=s[14]; s[14]=t;
    t=s[15]; s[15]=s[11]; s[11]=s[7]; s[7]=s[3]; s[3]=t;
}

static void inv_shift_rows(uint8_t *s) {
    uint8_t t;
    t=s[13]; s[13]=s[9]; s[9]=s[5]; s[5]=s[1]; s[1]=t;
    t=s[2]; s[2]=s[10]; s[10]=t; t=s[6]; s[6]=s[14]; s[14]=t;
    t=s[3]; s[3]=s[7]; s[7]=s[11]; s[11]=s[15]; s[15]=t;
}

static void mix_columns(uint8_t *s) {
    for(int i=0;i<4;i++){
        uint8_t *c=s+i*4;
        uint8_t a0=c[0],a1=c[1],a2=c[2],a3=c[3];
        uint8_t h=a0^a1^a2^a3;
        c[0]^=xtime(a0^a1)^h; c[1]^=xtime(a1^a2)^h;
        c[2]^=xtime(a2^a3)^h; c[3]^=xtime(a3^a0)^h;
    }
}

static uint8_t mul(uint8_t a, uint8_t b) {
    uint8_t p=0;
    for(int i=0;i<8;i++){
        if(b&1) p^=a;
        uint8_t hi=a&0x80;
        a<<=1; if(hi) a^=0x1b; b>>=1;
    }
    return p;
}

static void inv_mix_columns(uint8_t *s) {
    for(int i=0;i<4;i++){
        uint8_t *c=s+i*4;
        uint8_t a=c[0],b=c[1],g=c[2],d=c[3];
        c[0]=mul(a,14)^mul(b,11)^mul(g,13)^mul(d,9);
        c[1]=mul(a,9)^mul(b,14)^mul(g,11)^mul(d,13);
        c[2]=mul(a,13)^mul(b,9)^mul(g,14)^mul(d,11);
        c[3]=mul(a,11)^mul(b,13)^mul(g,9)^mul(d,14);
    }
}

static void add_round_key(uint8_t *s, const uint8_t *rk) {
    for(int i=0;i<16;i++) s[i]^=rk[i];
}

static void key_expansion(const uint8_t *key, uint8_t *rk) {
    for(int i=0;i<16;i++) rk[i]=key[i];
    for(int i=1;i<=10;i++){
        uint8_t *prev=rk+(i-1)*16, *cur=rk+i*16;
        uint8_t t[4]={prev[13],prev[14],prev[15],prev[12]};
        for(int j=0;j<4;j++) t[j]=sbox[t[j]];
        t[0]^=rcon[i];
        for(int j=0;j<4;j++){
            cur[j]=prev[j]^t[j];
            cur[4+j]=prev[4+j]^cur[j];
            cur[8+j]=prev[8+j]^cur[4+j];
            cur[12+j]=prev[12+j]^cur[8+j];
        }
    }
}

void AES_init_ctx_iv(AES_ctx* ctx, const uint8_t* key, const uint8_t* iv) {
    key_expansion(key, ctx->round_keys);
    /* Generate decryption keys (inverse mix columns on rounds 1-9) */
    for(int i=0;i<9;i++){
        for(int j=0;j<16;j++) ctx->dec_keys[i*16+j]=ctx->round_keys[(i+1)*16+j];
        inv_mix_columns(ctx->dec_keys+i*16);
    }
    for(int i=0;i<16;i++) ctx->iv[i]=iv[i];
}

void AES_ctx_set_iv(AES_ctx* ctx, const uint8_t* iv) {
    for(int i=0;i<16;i++) ctx->iv[i]=iv[i];
}

void AES_CBC_decrypt_buffer(AES_ctx* ctx, uint8_t* buf, size_t len) {
    size_t blocks=len>>4;
    uint8_t tmp[16];
    for(size_t b=0;b<blocks;b++){
        uint8_t *bl=buf+b*16;
        for(int i=0;i<16;i++) tmp[i]=bl[i];
        add_round_key(bl, ctx->round_keys+160);
        for(int r=9;r>=1;r--){
            inv_shift_rows(bl);
            inv_sub_bytes(bl);
            add_round_key(bl, ctx->round_keys+r*16);
            inv_mix_columns(bl);
        }
        inv_shift_rows(bl);
        inv_sub_bytes(bl);
        add_round_key(bl, ctx->round_keys);
        for(int i=0;i<16;i++) bl[i]^=ctx->iv[i];
        for(int i=0;i<16;i++) ctx->iv[i]=tmp[i];
    }
}

void AES_CBC_encrypt_buffer(AES_ctx* ctx, uint8_t* buf, size_t len) {
    size_t blocks=len>>4;
    for(size_t b=0;b<blocks;b++){
        uint8_t *bl=buf+b*16;
        for(int i=0;i<16;i++) bl[i]^=ctx->iv[i];
        add_round_key(bl, ctx->round_keys);
        for(int r=1;r<=9;r++){
            sub_bytes(bl);
            shift_rows(bl);
            mix_columns(bl);
            add_round_key(bl, ctx->round_keys+r*16);
        }
        sub_bytes(bl);
        shift_rows(bl);
        add_round_key(bl, ctx->round_keys+160);
        for(int i=0;i<16;i++) ctx->iv[i]=bl[i];
    }
}

#endif /* !AES_HW_AVAILABLE */
